...
$(document).ready(function($){
    ...
    function wcppa_getParcelas(target, order_id, acc, apihost){
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                for(var i = 0; i <= 5; i++){
                    if(typeof obj[i].total !== 'undefined'){
                        temparcde1a6++;
                        ...
                        html += '<li class="list-group-item box_cbk_parcela" parcela="'+obj[i].installment+'" valor="'+v_total+'"><strong>' + obj[i].installment + "X de R$ "+ v_mes + "</strong><br><span class='box_cbk_parcela_font2'>total de R$ " + v_total+'</span></li>';
                    }
                }
                html += '</ul>';
                if(temparcde1a6 > 0){
                ...
                var temparcde6a12 = 0;
                for(var i = 6; i <= 11; i++){
                    if(typeof obj2[i].total !== 'undefined'){
                        temparcde6a12++;
                        ...
                        html += '<li class="list-group-item box_cbk_parcela" parcela="'+obj2[i].installment+'" valor="'+v_total+'"><strong>' + obj2[i].installment + "X de R$ "+ v_mes + "</strong><br><span class='box_cbk_parcela_font2'>total de R$ " + v_total+'</span></li>';
                    }
                }
                html += '</ul>';
                if(temparcde6a12 > 0){
    ...
    function wcppa_getParcelasDolar(order_id, acc, apihost){
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                localStorage.setItem('LS_OBJ_JSON_PARCELAS_DOLAR', JSON.stringify(data));
            }
        });
    }
    ...
    function wcppa_iniJS()
    {
        jQuery(document).ready(function($){
            ...
            $( "#btn_finaliza_com_cartao" ).on( "click", function() {
                ...
                if(card_name == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>NOME COMPLETO</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_numero == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>NÚMERO DO CARTÃO</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_cvv == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>CVV - CÓDIGO DE SEGURANÇA</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_data_valid == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>DATA DE EXPIRAÇÃO</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_parcelas == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">Selecione uma <strong>PARCELA</strong>.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_cep == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>CEP</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_street == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>ENDEREÇO</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_street_number == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>NÚMERO</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_street_bairro == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>BAIRRO</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_street_city == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">O campo <strong>CIDADE</strong> não pode ser vazio.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(card_street_state == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">Selecione um <strong>ESTADO</strong>.</div>');
                    wcppa_hiddMsg(target, 3000);
                } else if(li_termo == ''){
                    $(target).html('<div class="alert alert-danger" role="alert">Confirme se você concorda com os <strong>TERMOS DE USO</strong> e <strong>POLÍTICA DE PRIVACIDADE</strong> da plataforma Parcelow.</div>');
                    wcppa_hiddMsg(target, 3000);
                }
                else{
            ...
            $('#card_numero').blur(function() {
                ...
                if(n != ''){
                    ...
                    if(ret == false){
                        $(target).html('<span style="color: #dc3545;"><i class="fa-solid fa-circle-exclamation"></i> Inválido.</span>');
                        wcppa_hiddMsg(target, 3000);
                        $(this).val('');
                    }
                }
            ...
            $('#card_data_valid').blur(function() {
                ...
                if(da != '' && da.length == 7){
                    ...
                    if(parseInt(anoexp) == parseInt(ano) && parseInt(mesexp) >= parseInt(mes)){
                        ret++;
                        //console.log('X1');
                    } else if(parseInt(anoexp) > parseInt(ano) && parseInt(mesexp) < parseInt(mes) && parseInt(mesexp) >= 1 && parseInt(mesexp) <= 12){
                        ret++;
                        //console.log('X2');
                    } else if(parseInt(anoexp) > parseInt(ano) && parseInt(mesexp) < parseInt(mes) && parseInt(mesexp) >= 1 && parseInt(mesexp) <= 12){
                        ret++;
            ...
            function wcppa_getCep(target, cep){
                ...
                jQuery.ajax({
                    ...
                    beforeSend: function () {
                        $(target).html('<div class="spinner-border spinner-border-sm" role="status"><span class="visually-hidden">Loading...</span></div>');
                    }
                });
            }
            ...
            function wcppa_responseQuestion(target)
            {
                ...
                if(vq1 == false){
                    $(target).html('<br><br><div class="alert alert-danger"><i class="bi bi-x-circle"></i> Marque 1 resposta da pergunta 1.</div><br><br>');
                } else if(vq2 == false){
                    $(target).html('<br><br><div class="alert alert-danger"><i class="bi bi-x-circle"></i> Marque 1 resposta da pergunta 2.</div><br><br>');
                } else{
